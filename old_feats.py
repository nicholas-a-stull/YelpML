...
def get_most_common_stars(instance):
    ...
    for i in range(0, len(instance["tokens"]) - 1):
        if instance["tokens"][i] == "1" and (
                instance["tokens"][i + 1] == "stars" or instance["tokens"][i + 1] == "star"):
            num_ones += 1
        elif instance["tokens"][i] == "2" and (
                instance["tokens"][i + 1] == "stars" or instance["tokens"][i + 1] == "star"):
            num_twos += 1
        elif instance["tokens"][i] == "3" and (
                instance["tokens"][i + 1] == "stars" or instance["tokens"][i + 1] == "star"):
            num_threes += 1
        elif instance["tokens"][i] == "4" and (
                instance["tokens"][i + 1] == "stars" or instance["tokens"][i + 1] == "star"):
            num_fours += 1
        elif instance["tokens"][i] == "5" and (
                instance["tokens"][i + 1] == "stars" or instance["tokens"][i + 1] == "star"):
            num_fives += 1
    highest_val = max(num_ones, num_twos, num_threes, num_fours, num_fives)
    if highest_val == num_ones:
